...
class DB
{
    ...
    function DB($base, $server, $user, $pass)
    {
      ...
      if(IS_MYSQLI_ON)
      {
            $dbLink = mysqli_connect($server, $user, $pass) or die(mysqli_connect_error());
            mysqli_select_db($dbLink,$base)               or die(mysqli_error($dbLink));
            $db_link = $dbLink;
      }
      else
      {
    ...
    {
      $this->nbQueries++;
      $this->lastResult = DB_queryFunc($query) or $this->debugAndDie($query);
    ...
    {
      $this->nbQueries++;
      DB_queryFunc($query) or $this->debugAndDie($query);
	  //$this->lastInsertedId();
     //echo "===>". //$this->debug($debug, $query);
    }
    ...
    function fetchNextObject($result = NULL)
    {
      ...
      if ($result == NULL || DB_numRowsFunc($result) < 1)
        return NULL;
      else
        return DB_fetchObjectFunc($result);
    }
    ...
    function fetchNextAssoc($result = NULL)
    {
      ...
      if ($result == NULL || DB_numRowsFunc($result) < 1)
        ...
        return DB_fetchAssocFunc($result);
    }
    ...
    {
      if ($result == NULL)
        return DB_numRowsFunc($this->lastResult);
      else
	    return DB_numRowsFunc($result);
    }
    ...
    function queryUniqueObject($query, $debug = -1)
    {
      $query = "$query LIMIT 1";
      $this->nbQueries++;
      $result = DB_queryFunc($query) or $this->debugAndDie($query);
    ...
    function debugAndDie($query)
    {
      ...
      die("<p style=\"margin: 2px;\">".DB_errorFunc()."</p></div>");
    }
    ...
    function debug($debug, $query, $result = NULL)
    {
      ...
      if ($result == NULL)
        echo "<p style=\"margin: 2px;\">Number of affected rows: ".DB_affectedRowFunc()."</p></div>";
      else
    ...
    function debugResult($result)
    {
      ...
      $numFields = DB_numFieldsFunc($result);
      // BEGIN HEADER
      $tables    = array();
      $nbTables  = -1;
      $lastTable = "";
      ...
      while ($column = DB_fetchFieldsFunc($result)) {
      ...
      while ($row = DB_fetchArrayFunc($result)) {
        echo "<tr>";
    ...
    {
      if (@DB_numRowsFunc($result) > 0)
        DB_dataSeekFunc($result, 0);
    }
