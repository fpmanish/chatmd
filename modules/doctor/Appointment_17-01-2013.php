...
$(document).ready(function(){
	$("#timeCalendar").continuousCalendar({ firstDate:"today" , selectToday: false, theme: 'rounded',"customScroll":true});
$("#timeCalendar").bind('calendarChange', function() {
  ...
  var duration=  $(".totalTimeOfTrip").html();
...
return dat.getMonth();
 ...
 $("#starts").val("<?php echo date('G:i',$calenderList[0]['start_time']); ?>");
	$("#ends").val('<?php echo date('G:i',$calenderList[0]['end_time']); ?>');
 	$(".rangeLengthLabel").html('<?php echo $numDyas." days"?>');
 	$(".startDateLabel").html('<?php echo $startWeekDAy.  $startDate; ?>');
 	$(".endDateLabel").html('<?php echo $endWeekDay.  $endtDate ;?>');
 	var duration=  $(".totalTimeOfTrip").html();
 	$("#duration").val(duration);
  $('.calendarBody  tr>td').each(function() {
 ...
 <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" id="test">
  <div id="timeCalendar">
    ...
    <div class="dateLabels">
      ...
      <input type="text" name="timeInterval" value="<?php echo $calenderList[0]['interval']; ?>" placeholder="Mintees"  maxlength="2">
      </div>
      <div>
        <label>Duration</label>
        <span class="totalTimeOfTrip"><?php echo $calenderList[0]['duration']; ?></span>
      </div>
      <div>
      	<input type="hidden" name="duration" id="duration">
         <input type="hidden" name="<?php if(empty($calenderList)) { echo "app"; }else { echo "Editapp";}?>" value="1">
