...
$EmailCount=$settingsObj->getPatientByName($email);
 $EmailTemplate=$pageObj->getEmailById(2);
...
if($Patient_reg=="save_patient" && $pat_submit !=""){
...
$dataArr=array("name"=>$full_name,"email"=>$email,"password"=>$password,"dob"=>$date_str,
                "gender"=>$radio_1_set,"add_date"=>$currentTimestamp,
                "modify_date"=>$currentTimestamp,"is_active"=>1,"user_type"=>0);
...
$newphrase = str_replace($find, $repalce, $phrase);
 if($sucess>0){
					...
					$mailObj->From = "info@chat-md.com";										$mailObj->FromName = "Chat-MD";
...
$error_message="Mailer Error: " . $mailObj->ErrorInfo; 
					 	...
					 	$_SESSION['Regististrstion']="chandralog";
						 header("Location: ".MODULE_URL."/home");
						 	 exit;
