...
$pageType = "forget";
$user_array=$settingsObj->getPatientByName($email);
if(isset($_SESSION['AD_admin_LoggedIn'])) {
	
	 header("location:".MODULE_URL."/home");
                exit;
}
 $EmailTemplate=$pageObj->getEmailById(3);
 
if($email !="" && $Femail ==1 && $id=="")
{
if(count($user_array)>0)
{
	...
	// $diffNum2=rand(0,100);
	 ...
	 $url= MODULE_URL.'/forgetPassword/index.php?recoveryPasswor=yes&id='.$user_array['patient_id'];
$find = array("%username%", "%URL%");
$repalce   = array(ucfirst($user_array['name']),$url);
	...
	<tr>
		<td width="100%" valign="top">
			...
			<table width="590" border="0" cellpadding="0" cellspacing="0" bgcolor="#ffffff" align="center" class="scaleForMobile" style="border-left: 1px solid #e9e9e9; border-right: 1px solid #e9e9e9;">
                ...
                <tr>
                	<td style="text-align:left; padding:20px 0 15px 20px;font-family:Arial, Helvetica, sans-serif; font-size:12px;">Yours sincerely,</td>
                </tr>
			</table>
...
$error_message="Thi Email is not exists";	
}
}
if($recoveryPass ==1)
{
...
{  
	...
	$settingsObj->updatePatient($id,$dataArr);
	 header("Location: ".MODULE_URL."/Login");
						 	 exit;
}
}
