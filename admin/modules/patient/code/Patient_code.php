...
$loginObj->isLoggedIn();
$pageNameMenu = "patient";
$PagetypeText = "Add";
$settingObj = new settings();
$pageObj = new pageManager();
$mailObj = new PHPMailer();
$patientList = $settingObj->patientList();
 $currentTimestamp = getCurrentTimestamp();
 
if((count($_POST)) && $pageType != "list")   // for add/edit/delete condition.
{
if($pageType == "addPatient" && $Patient_id == "")      // Add Page
    {
    	$EmailTemplate=$pageObj->getEmailById(4);
    	 $PagetypeText = "Add";
  ...
  $dataArr=array("name"=>$name,"email"=>$email,"password"=>$Epassword,"dob"=>$date_str,
                ...
                "modify_date"=>$currentTimestamp,"is_active"=>$activate,"user_type"=>'0');

    $success=  $settingObj->PatientAdd($dataArr);
	$diffNum=genRandomPass(3);
	 $diffNum2=genRandomPass(2);
	 $phrase  = $EmailTemplate['template_content'];
	 $url= MODULE_URL.'/Login/index.php?sueccess='.$diffNum.$success.$diffNum2;
...
$error_message="Mailer Error: " . $mailObj->ErrorInfo; 
					  ...
					  header("location: ".ADMIN_MODULE_URL."/patient/index.php");
         exit();
 ...
 else if($pageType == "editPatient" && $patient_id != "")    // Edit page
    {
  ...
  if($password !=""){
	  ...
	  $dataArr=array("name"=>$name,"email"=>$email,"dob"=>$date_str,"password"=>$password,
  	 ...
  	 $dataArr=array("name"=>$name,"email"=>$email,"dob"=>$date_str,
			...
			//  echo "<pre>";
			 // print_r($dataArr); die;
      $settingObj->updatePatient($patient_id,$dataArr);
	  header("location: ".ADMIN_MODULE_URL."/patient/index.php");
         exit();
...
if(count($_GET) && $pageType == "editPatient" && $page_id != "")     // Edit data fetch.
{
    $PagetypeText = "Edit";
  ...
  $patientData=$settingObj->UpdatePatientById($page_id);
	...
	else if($isUserAct !="" && $id != "") {
		
		$settingObj->actDeactuser($id,$isUserAct);
	    header("location: ".ADMIN_MODULE_URL."/patient/index.php");
		exit();
	}	
