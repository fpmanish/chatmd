...
if($pageType == "addDoctor" && $Patient_id == "")      // Add Page
 ...
 $sucess=$settingObj->PatientAdd($dataArr);
 				$newArr=array("phone"=>$Phone,"Degrees"=>$Degrees,"Experience"=>$Experience,"registration_id"=>$sucess,"is_accepted"=>$New_1_set,"hospital"=>$hospital,"CountryId"=>$Country,"StateId"=>$State,"CityId"=>$City);
 if(count($langauge)>0)
	{
...
$error_message="Mailer Error: " . $mailObj->ErrorInfo; 
  ...
  if($password !=""){
	  ...
	  $dataArr=array("name"=>$name,"email"=>$email,"dob"=>$date_str,"password"=>$password,
                "gender"=>$gender,"add_date"=>$currentTimestamp,
                "modify_date"=>$currentTimestamp,"is_active"=>$activate,"user_type"=>'1');
				$newUpdateArr=array("phone"=>$Phone,"Degrees"=>$Degrees,"Experience"=>$Experience,"is_accepted"=>$New_1_set,"hospital"=>$hospital,"CountryId"=>$Country,"StateId"=>$State,"CityId"=>$City);
  }else{
